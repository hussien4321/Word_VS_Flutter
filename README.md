...
**Key libraries** · `flutter_bloc` · `bloc_concurrency` · `freezed` + `freezed_annotation` · `json_serializable` · `cloud_firestore` · `firebase_core` · `get` · `shared_preferences` · `confetti` · `intl`

~26k lines of Dart · pure game engine, no framework coupling
...
|---|---|---|
| **Scoring duplicate letters** | The rule everyone gets wrong. `SPEED` guessed against `ERASE` must mark exactly as many yellows as the answer actually has letters to spend. Needs two passes and a consumed-letter budget. | [↓](#1-the-duplicate-letter-problem) |
| **A game engine with no framework in it** | `WordleeGame` imports no Flutter, no Firebase and no BLoC. It's a plain Dart object the UI drives, which is what makes the rules testable and the two modes share them. | [↓](#2-one-engine-two-modes) |
| **Two players, one answer, no server** | Head-to-head runs on a single Firestore document with a collision-checked room code. Neither client is authoritative over the other's board. | [↓](#3-head-to-head-on-one-document) |
| **States that can't be misread** | Game state, results and settings are `freezed` sealed unions, so "in progress" and "finished with a result" can't be confused at a call site. | [↓](#4-sealed-state-with-freezed) |
...
### 1. The duplicate-letter problem
...
### 3. Head-to-head on one document
...
- **Stale-session cleanup lives outside this repo.** Sessions store `createdAt` for it, but the expiry itself is configured in the Firebase console rather than as code here — so it isn't reviewable alongside the thing it cleans up.
- **No tests.** The engine and the guess scorer are pure functions with obvious edge cases; they're exactly what should have been tested and weren't.
- **Room codes race.** The check-then-claim in `_createNewRoomID` isn't atomic. The code space makes a collision unlikely rather than impossible; a transaction would close it.
- **Word list ships in the bundle.** Fine for a fixed dictionary, but the list can't change without a release.
